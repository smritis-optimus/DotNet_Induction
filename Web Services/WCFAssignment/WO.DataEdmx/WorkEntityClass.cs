            ...
            try
            {
                ...
                foreach (BE.Operation operations in workOrder.Operations)
                {
                    ...
                    if (operations.OpStatus == "Open")//if status is open set null to OnCompleteeDate
                       operation.OpCompleteDate = null;
                    else
                      operation.OpCompleteDate = Convert.ToDateTime(operations.OpCompleteDate);
                ...
                using (WorkDataEntities updateWork = new WorkDataEntities())
                {
                    ...
                    foreach (BE.Operation operations in workOrder.Operations)
                    {
                        ...
                        if (operations.OpStatus != null)
                            queryOperation.OpenStatus = operations.OpStatus;
                        if (operations.OpCompleteDate != null)
                            queryOperation.OpCompleteDate = Convert.ToDateTime(operations.OpCompleteDate);
                    }
